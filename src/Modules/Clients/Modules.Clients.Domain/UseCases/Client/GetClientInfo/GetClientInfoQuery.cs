...
namespace Modules.Clients.UseCases.Client.GetClientInfo
{
    ...
    internal sealed class GetClientInfoQueryHandler(IDoctorService doctorService) : IRequestHandler<GetClientInfoQuery, Result<GetClientInfoRes>>
    {
        public async Task<Result<GetClientInfoRes>> Handle(GetClientInfoQuery request, CancellationToken cancellationToken)
        {
            var doctorOfClientRes = await doctorService.GetDoctorInfoById(new Doctors.ApiContracts.Doctor.Dtos.GetDoctorInfoByIdDtoRequest
            {
                Id = 44
            });
